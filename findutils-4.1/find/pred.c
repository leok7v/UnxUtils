...
static myboolean insert_lname P_((char *pathname, struct stat *stat_buf, struct predicate *pred_ptr, myboolean ignore_case));
static myboolean launch P_((struct predicate *pred_ptr));
...
//  child_pid = fork ();
    getcwd(this_pwd,sizeof(this_pwd));  
...
//child_pid=_spawnvp (_P_NOWAIT, execp->vec[0], execp->vec);
child_pid=Downhill_Process_Forkexec(execp->vec[0], execp->vec, NULL ,0); 

  if (child_pid == -1)
    error (1, errno, "cannot fork");
  if (child_pid == 0) // never
    {
      /* We be the child. */
#ifndef HAVE_FCHDIR
      if (chdir (starting_dir) < 0)
...
//wait_ret=_cwait (&status, child_pid,_WAIT_CHILD );;
  chdir (this_pwd);
